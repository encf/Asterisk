...
### Table II: integer multiplications
...
- Notes: this runs the paper's multiplication grid under Net-L / Net-H with `n=5,10,16` and 10,000 dependent multiplications.
...
- Notes: this runs the paper's comparison grid for a single comparison under Net-L / Net-H with `n=5,10,16`.

### Table III: fixed-point multiplications
...
- Notes: the defaults now match the paper's Asterisk 2.0 rows: `n=5` and 1,000 consecutive fixed-point multiplications.
...
- Notes: this runs the paper's truncation grid for Net-L / Net-H with `n=5,10,16`, batch size 1,000, and single-latency repeat 5.
...
- Notes: this runs the paper's VM grid with `M=N=32`, unit order size, and `n=5,10,16`.
...
- Notes: this runs the paper's CDA grid with `M=N=50` and `n=5,10,16`.
