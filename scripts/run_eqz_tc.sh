...
EQZ_SCRIPT="${ROOT_DIR}/scripts/compare_eqz_a2.sh"
OUT_DIR="${ROOT_DIR}/run_logs/eqz_tc"
...
usage() {
  cat <<'EOF'
Usage: scripts/run_eqz_tc.sh [options]

Configure loopback tc/netem, run the standalone EQZ benchmark, and save the
...
Options:
  ...
  --out-dir <path>              Output directory (default: run_logs/eqz_tc)
...
validate_port_range() {
  ...
  local max_port_needed=$((port + 2 * case_stride - 1))
...
if [[ ! -x "${EQZ_SCRIPT}" ]]; then
  echo "Expected executable EQZ script at ${EQZ_SCRIPT}" >&2
  exit 1
fi
...
if [[ -z "${LABEL}" ]]; then
  LABEL="eqz_owd${ONE_WAY_DELAY_MS}ms_n${N}"
fi
...
"${eqz_cmd[@]}" | tee "${RUN_DIR}/compare_output.txt"

echo
echo "[DONE] EQZ tc run saved to: ${RUN_DIR}"
