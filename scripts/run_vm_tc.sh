...
COMPARE_SCRIPT="${ROOT_DIR}/scripts/compare_vm_protocols.sh"
OUT_DIR="${ROOT_DIR}/run_logs/vm_tc"
...
usage() {
  cat <<'EOF'
Usage: scripts/run_vm_tc.sh [options]

Configure loopback tc/netem, run the VM benchmark comparison, and save the
...
Options:
  ...
  --out-dir <path>              Output directory (default: run_logs/vm_tc)
...
if [[ -z "${LABEL}" ]]; then
  LABEL="vm_owd${ONE_WAY_DELAY_MS}ms_n${N}"
fi
...
echo
echo "[DONE] VM tc run saved to: ${RUN_DIR}"
