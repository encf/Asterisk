...
BASE_PORT=""
LABEL=""
OUT_DIR="${ROOT_DIR}/run_logs/eqz_check"

usage() {
  cat <<'EOF'
Usage: scripts/check_eqz_a2.sh --x <int> [options]

Run Asterisk2.0 EQZ locally and reconstruct the final output bit.
...
Options:
  ...
  -o, --out-dir <path>         Output directory (default: run_logs/eqz_check)
  -h, --help                   Show help
EOF
}
...
for base in range(START, END - WIDTH + 1, STRIDE):
    ...
    raise SystemExit("Could not find a free port range for EQZ check")
PY
}
...
if [[ -n "${LABEL}" ]]; then
  ...
  RUN_OUT_DIR="${OUT_DIR}/x_${X_CLEAR}_${SECURITY_MODEL}_n${N}_$(date +%Y%m%d_%H%M%S)"
fi
...
echo "[RUN] EQZ check: x=${X_CLEAR}, n=${N}, model=${SECURITY_MODEL}, lx=${LX}, slack=${SLACK}, repeat=${REPEAT}, port=${BASE_PORT}"
...
python3 - "${RUN_OUT_DIR}" "${N}" "${REPEAT}" "${X_CLEAR}" "${SECURITY_MODEL}" "${FIELD_PRIME}" <<'PY'
import json
import pathlib
import sys
...
print(f"[INFO] expected EQZ({x_clear}) = {expected}")
all_ok = True
for r in range(repeat):
    ...
    ok = (out == expected)
    line = f"[CHECK] repeat={r} reconstructed={out} expected={expected} -> {'PASS' if ok else 'FAIL'}"
    if security_model == "malicious":
