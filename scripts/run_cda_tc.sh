...
COMPARE_SCRIPT="${ROOT_DIR}/scripts/compare_cda_protocols.sh"
OUT_DIR="${ROOT_DIR}/run_logs/cda_tc"
...
usage() {
  cat <<'EOF'
Usage: scripts/run_cda_tc.sh [options]

Configure loopback tc/netem, run the CDA benchmark comparison, and save the
...
Options:
  ...
  --slack <int>                 Asterisk2.0 comparison slack parameter (default: 8)
  ...
  --out-dir <path>              Output directory (default: run_logs/cda_tc)
...
validate_port_range() {
  ...
  local max_port_needed=$((port + 3 * case_stride - 1))
...
if [[ -z "${LABEL}" ]]; then
  LABEL="cda_owd${ONE_WAY_DELAY_MS}ms_n${N}"
fi
...
if [[ "${SKIP_TC}" -eq 0 ]]; then
  ...
  PING_AVG_MS="$(measure_ping_avg_ms "${RUN_DIR}/ping.txt")"
  {
    ...
    echo "num_parties=${N}"
    echo "buy_list_size=${BUY_SIZE}"
    echo "sell_list_size=${SELL_SIZE}"
...
echo
echo "[DONE] CDA tc run saved to: ${RUN_DIR}"
