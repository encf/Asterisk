...
void benchmark(const bpo::variables_map& opts) {
  ...
  if (!opts["localhost"].as<bool>()) {
    throw std::runtime_error("EQZ benchmark currently supports localhost only");
  }
  ...
  for (size_t r = 0; r < repeat; ++r) {
    ...
    asterisk2::EqzOfflineData eqz_offline;
    asterisk2::EqzOfflineDataMalicious eqz_offline_mal;
    if (security_model == asterisk2::SecurityModel::kSemiHonest) {
      eqz_offline = proto.eqz_offline(lx, slack);
    } else {
      eqz_offline_mal = proto.eqz_offline_malicious(lx, slack);
    }
    ...
    if (security_model == asterisk2::SecurityModel::kSemiHonest) {
      out_share = proto.eqz_online(x_share, eqz_offline);
    } else {
      auto auth_out = proto.eqz_online_malicious(local_x, local_dx, eqz_offline_mal);
      out_share = auth_out.eqz_share;
      delta_out_share = auth_out.delta_eqz_share;
      delta_share = eqz_offline_mal.nonneg_data.delta_share;
    }
...
bpo::options_description programOptions() {
  bpo::options_description desc("Asterisk2.0 EQZ benchmark options");
  desc.add_options()
...
int main(int argc, char* argv[]) {
  ...
  bpo::options_description cmdline("Benchmark Asterisk2.0 EQZ protocol.");
